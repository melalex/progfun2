...
trait Solver extends GameDef {
  ...
  def from(initial: LazyList[(Block, List[Move])],
    ...
    if (initial.isEmpty) LazyList.empty
      ...
      val more = newNeighborsOnly(
        initial.flatMap { case (block, history) => neighborsWithHistory(block, history) },
        explored
      )
   ...
   */
  lazy val pathsToGoal: LazyList[(Block, List[Move])] = pathsFromStart.filter { case (block, _) => done(block) }
   ...
   * position.
   */
  lazy val solution: List[Move] = pathsToGoal.map(_._2).head.reverse
}
